...
-- Cace 4
SELECT * FROM ms_produk_dqlab WHERE nama_produk IN ('Flashdisk DQLab 84 GB', 'Tas Travel Organizer DigiSkills.id', 'Gantungan Kunci DQLab');

-- Case 5
SELECT * FROM ms_produk_dqlab WHERE harga < 50000;

-- Case 6
SELECT kode_pelanggan, nama_produk, qty, harga, (qty*harga) AS total FROM tr_penjualan_dqlab WHERE (qty*harga) >= 200000
ORDER BY total DESC;
...
-- Case 8
SELECT kode_transaksi, tgl_transaksi, no_urut, kode_produk,nama_produk, CONCAT(kode_produk, ' - ', nama_produk) AS kode_produk_namaproduk
FROM tr_penjualan_dqlab WHERE qty = 5;

-- Case 9
SELECT nama_pelanggan,
TRIM(REPLACE(nama_pelanggan, ', S.H', '')) AS nama_tanpa_gelar, SUBSTRING_INDEX(nama_pelanggan, ' ', 1) AS nama_panggilan FROM ms_pelanggan_dqlab
...
FROM  ms_pelanggan_dqlab p JOIN  tr_penjualan_dqlab t ON p.kode_pelanggan = t.kode_pelanggan
...
-- Case 18
SELECT nama_produk, no_urut FROM ms_produk_dqlab UNION SELECT nama_produk, no_urut FROM tr_penjualan_dqlab;

-- Case 19
SELECT p.kode_pelanggan, p.nama_pelanggan, p.alamat, pr.nama_produk FROM ms_pelanggan_dqlab p
JOIN tr_penjualan_dqlab t ON p.kode_pelanggan = t.kode_pelanggan JOIN ms_produk_dqlab pr ON t.kode_produk = pr.kode_produk
...
-- Case 21
SELECT nama_produk, tgl_transaksi, DAY(tgl_transaksi) AS day_date, MONTH(tgl_transaksi) AS month_date, YEAR(tgl_transaksi) AS year_date, DATEDIFF(CURDATE(), tgl_transaksi) AS selisih_hari
FROM tr_penjualan_dqlab;

-- Case 22
SELECT nama_produk, tgl_transaksi, DATE_FORMAT(tgl_transaksi, '%M') AS month_date, YEAR(tgl_transaksi) AS year_date, 
