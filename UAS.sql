...
SELECT p.kategori_produk, SUM(t.qty * (t.harga - (t.diskon_persen/100) * t.harga)) AS revenue,
       CASE 
           WHEN SUM(t.qty * (t.harga - (t.diskon_persen/100) * t.harga)) >= 900000 THEN 'Target Achieved'
           WHEN SUM(t.qty * (t.harga - (t.diskon_persen/100) * t.harga)) < 900000 THEN 'Less Perfomance'
           ELSE 'Follow Up'
       END AS remark
